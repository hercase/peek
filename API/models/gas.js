...
ReadComprobantes: function (oData, oResponse) {
    var sSQLRead = 'CALL usp_get_gas_comprobantes_by_id(?)';
    ...
    oMyConnection.query(sSQLRead, data, function(oError, oRows, oCols) {
      if( oRows[0].length === 0)
      {
        ...
        //   error: true,
        //   error_object: "Usuario Inexistente"
        // }));
  ...
  ReadUser: function (oData, oResponse) {
    var sSQLRead = 'CALL usp_get_gas_ctas_by_id(?)';
    var data = [oData.account_id];
    oMyConnection.query(sSQLRead, data, function(oError, oRows, oCols) {
      if( oRows[0].length === 0)
      {
      ...
      else
      {
        if(oError) {
          ...
          if(id > 0) {
            ...
            var data = [id, oData.servicio_id, new Date()];
            var oResp = oRows;
            oMyConnection.query(sSQLRead, data, function(oError, oRows, oCols) {
              if(oError) {
                oResponse.write(JSON.stringify({
  ...
  AddEmail: function(oData, oResponse) {  
      var sSQLCreate = "CALL usp_create_gas_ctas_email(?,?)";
  ...
  ReadEmails: function (oData, oResponse) {
    var sSQLRead = 'CALL usp_get_gas_ctas_email_by_id(?)';
    var data = [oData.account_id];
    oMyConnection.query(sSQLRead, data, function(oError, oRows, oCols) {
      if( oRows[0].length === 0)
      {
  ...
  UpdateEmail: function (oData, oResponse) {
    var sSQLRead = 'CALL usp_update_gas_ctas_email_by_id(?,?,?,?)';
  ...
  UpdateUser: function (oData, oResponse) {
    var sSQLRead = 'CALL usp_update_gas_cta_by_id(?,?)';
